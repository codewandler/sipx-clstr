//! `DP-13` — a node performs only the methods its declared roles wire (`cluster-config` §4 R3).
//!
...
/// **The failing-first test for `DP-13`.** A node that is not a registrar does not register anyone.
///
...
async fn dp13_an_inbound_proxy_does_not_register_anyone() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn dp13_a_registrar_does_not_proxy() {
    let sink = UdpSocket::bind("127.0.0.1:0").await.expect("bind the sink");
    let sink_port = sink.local_addr().expect("the sink's address").port();
    let caller = UdpSocket::bind("127.0.0.1:0")
        .await
...
#[test]
fn dp13_a_role_this_build_cannot_serve_stops_the_node() {
