...
| [cluster-config](cluster-config.md) §8, beside V9 | **Done by `FC-8`.** KY3's redaction requirement is not a property of keys — it is a property of `ConfigError`, which §8 declares, and it binds `dsnRef`, `keyRef` and `tenant[].auth.secret` identically. V9 now permits a description of what was written or no `found` value, and forbids the value itself in the error, process log, operator admission response, or any future diagnostic derived from the refusal. |
