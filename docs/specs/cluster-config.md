...
- This repo's specs consumed, each of which owns the content of the section §7 assigns it:
  ...
  §6.2 C2 (the edge id that is also §6's node id);
...
| D3 | **Two encodings, one data model:** YAML 1.2 (core schema) and JSON. The normative artefact is the typed tree, not either spelling; a loader MUST produce identical `Config` values for two encodings of the same tree. |
...
| A status code the platform may choose where a spec already fixed one | V6. A configurable `404`/`503` is a deployment quietly disagreeing with a normative rule. |
