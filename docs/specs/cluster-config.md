...
| V9 | **No secret value appears in the document or in a refusal derived from it.** Key material, database credentials and TLS private keys are named by reference — `dsnRef`, `secretRef`, `keyRef` — and resolved by the driver into the value the owning spec describes (a `keys` entry's `secret` is `affinity-token` §6's exactly-32-bytes *after* resolution). The document is rendered into a ConfigMap and the references into Secrets by the operator; a schema that permitted an inline secret would make the safe shape the optional one. When a value is written inline where one of those references belongs, `ConfigError.found` carries either a description of what was written or `None`, never the value. Every consumer of that refusal — the node's process log, an operator admission response, or any future diagnostic surface — MUST use the redacted rendering; changing the destination does not make the value safe to print. A reference that does not resolve is a **start-up** failure of the driver, not a load error, because resolution is IO (D1). |
...
| CC-V-25 | Distinct sentinel values are written at the inline neighbours of `dsnRef`, both `secretRef` fields, and `keyRef`: `locationStore.dsn`, `tenant[].auth.secret`, `keys[].secret`, `listener[].tls.key`, and `management.tls.key` | Each is refused at its own path under `CC-V9`; its `found` is a description or absent, and no sentinel occurs in the `ConfigError`, stable startup message, stdout, or stderr |
| CC-V-26 | A `SipxCluster` admission request carries the same five inline sentinels | Admission rejects it with an error response naming every offending path and containing none of the sentinels; no object is created and no existing object's `status` changes (A2). Deferred to `KO-3`, which implements admission and its error response |
