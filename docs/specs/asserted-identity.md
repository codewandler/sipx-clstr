...
identity selection (§7); the emission gate (§8); anonymous callers, the `From` header and the
`user`-level header deletions on both requests and responses (§9);
...
pub struct IdentityFacts {
    ...
    /// **The caller's privacy request for this dialog, not this message's header.** On a
    /// dialog-forming request it is parsed from the `Privacy` header. On an in-dialog request
    /// carrying no `Privacy` header it is reconstituted from A12's token fact, because a UA need
    /// not repeat the header. This is the single place that reconciliation happens, which is why
    /// §8's gate and §9's `From` rule can both read a plain value and why A24 stays total (A12).
...
| A5 | **The assertable set is what the platform will vouch for, and it is bounded at ingress.** For an authenticated request it is the set of identities the deployment holds for that principal ([registrar-auth](registrar-auth.md) §5); for an unauthenticated one admitted by source address (RT-8) it is empty. Every member satisfies RFC 3324 §2.2 — a `sip`/`sips` URI's domain, and the owner of a `tel` URI's E.164 number, are within this deployment's trust domain — **because of where the set comes from, not because this spec checks it**: RG-1's store holds identities the deployment issued to that principal, so membership is established by the same act that made them assertable. An `IdentitySource::Literal` is the one value that does *not* come from there, which is why it is the one place RFC 3324 §2.2 becomes an operator obligation rather than a property of the source (§12, `A17`). Where the set comes from is RG-1's and DP-1's; that it is resolved before egress and never after is this spec's. |
...
| # | Rule |
|---|---|
| A8 | **Three egress steps, in this order, inside the window `N23` already reserves.** Per branch, after [proxy-behavior](proxy-behavior.md) §7 F2 (Request-URI ← target) and before F5 (`BeforeForward`, H9): <br/>**E1 — identity** (§7, §8): decide whether a `P-Asserted-Identity` exists on this branch and whose identity it names; create, replace or remove accordingly. <br/>**E2 — normalisation** (`N23`): the trunk's profile shapes whatever numbers remain. <br/>**E3 — presentation** (§9): the `From` form of RFC 5379 §5.1.4, the `user`-level header deletions of A33, and the outgoing `Privacy` header. <br/>E1 therefore settles the PAI question **before** `N23` runs, and E3 writes its constants **after**. A33's deletions are order-independent with respect to `N23` — a header that is gone cannot be normalised, and none of the six carries a number the profile shapes — so they are placed in E3 with the rest of the `user` treatment rather than split across the boundary. On a response, E1 reduces to §8's emission gate alone — *synthesis* never runs, which is A13's rule and the reason `(§7, §8)` above is two references and not one — and E3 runs in the reduced form A35 fixes. A11 states the same boundary from the gate's side. |
...
| # | Rule |
|---|---|
| A19 | **`Assert::Never` creates nothing and removes nothing.** It is a statement about synthesis alone. Whether a `P-Asserted-Identity` already in hand travels is decided by A20–A24, exactly as it is for a trunk that does assert. **No configuration in this spec guarantees that no `P-Asserted-Identity` reaches a peer**, and an operator under a contract that requires that guarantee must know it: `trust: untrusted` with `when_unspecified: remove` is the strongest posture available here, and A22 still emits under `Privacy: none`, because RFC 3323 §4.2 and RFC 3325 §7 make `none` non-overridable. **No mechanism anywhere in the platform provides that guarantee today, either.** RT-5's per-trunk egress header allowlist is the obvious candidate and it does not reach: it is a default-deny over headers matching the platform's *application-header prefix* (RT-5's Acceptance; [extension-framework](../designs/extension-framework.md) "Approach" calls it "default-deny over a prefix"), and `P-Asserted-Identity` carries no such prefix, so an allowlist omitting it removes nothing. An operator under a contract requiring "this carrier never receives an asserted identity" must be told that the platform cannot enforce it, rather than sold a posture that `Privacy: none` defeats. §14 records the gap and what closing it would take; it is deliberately **not** patched here by making a privacy default override `none`, because RFC 3323 §4.2's `MUST NOT` is not a default's to override — the control that overrides an explicit caller request belongs where a reviewer sees an unconditional header removal, not inferred from a field named for the case where the caller said nothing. |
...
| A33 | **Performing `user` means every entry in Table 1's `user` column, and the column is enumerated here so nothing hides in it.** RFC 5379 §4.1 Table 1 gives `user` **eleven** entries; RFC 3323 §5.3 supplies the `MUST` behind six of them ("the privacy service MUST remove any non-essential informational headers that have been added by the user agent, including the Subject, Call-Info, Organization, User-Agent, Reply-To and In-Reply-To"). All eleven are accounted for below — **nine performed, two declined with a reason** — because a rule that silently drops entries is the defect this rule exists to prevent. `UserPrivacy::Perform` does all nine; `UserPrivacy::Decline` does none. There is no middle value: anonymising the `From` while forwarding headers that name the caller's software, employer and subject line is a claim to have provided user-level privacy that is false, and it is the value A29 would otherwise let a trunk advertise as performable. <br/>**Performed on requests (Table 1 `R`/`Rr`, seven):** `From: anonymize` (A26); `Subject`, `Call-Info`, `Organization`, `User-Agent`, `Reply-To`, `In-Reply-To` — `delete`. <br/>**Performed on responses (Table 1 `r`/`Rr`, five — three of them the same headers): A35.** <br/>**Declined, with the reason, both `R`:** `Call-ID: anonymize` is a `MAY` in RFC 3323 §5.3 ("MAY entail the modification of the Call-ID header") which §5.3 then puts behind a B2BUA and a restored-value store, and Table 1 flags it `(Note)`; `Referred-By: anonymize*` carries §4.1's asterisk — "the involvement of a privacy service … depend[s] on the circumstance". Declining a `MAY` and a circumstance-dependent `*` is not a failure to perform the level; declining the `MUST` list would be. <br/>**There is no seam with RT-5 to manage**: its allowlist is scoped to the platform's application-header prefix, and every header named here is a standard SIP header carrying no such prefix, so the two field sets are disjoint. Were they ever to overlap, both mechanisms only ever *remove*, so they would compose with no precedence rule — but that is a property to rely on, not a seam to specify. |
| A35 | **User-level privacy reaches responses, and it is gated by the response's own `Privacy` header — the same rule A13 fixed for `P-Asserted-Identity`, for the same reason.** Table 1 marks `Server` and `Warning` `r`, and `Call-Info`, `Organization` and `Reply-To` `Rr`, so five of the `user` entries are reachable on a response. On a response carrying `Privacy: user`, a trunk with `user_privacy: perform` **deletes `Server`** — RFC 5379 §5.1.12: "A privacy service must delete a Server header in a response when user privacy is requested with Privacy:user" — **anonymises `Warning` by deleting the hostname portion** where that hostname represents the UAS's identity (§5.1.16; the warn-code and warn-text are untouched, and a `Warning` carrying no hostname is forwarded byte-identical), and **deletes `Call-Info`, `Organization` and `Reply-To`**. The `From` is not touched on a response: Table 1 marks it `R`. <br/>**Why the response's own header and not the request's.** These five name the *responding* party's software, organisation and hostname — §5.1.12's own first sentence is "information about the software used by the **UAS**". A caller's `Privacy: user` is a statement about the caller, and reading it here would let one party's privacy request strip another party's headers. This is A13's argument with the subject changed, and it buys the same property: the response path reads only the message in front of it, needs no dialog state and no correlation to the request, so invariant 5 holds on responses without a store. <br/>**`critical` does not apply here**, and that is RFC 3323 §4.2's own instruction — "Criticality cannot be managed appropriately for responses" — so §10 does not run on a response (A13) and a response is never rejected for an unperformable value. |
...
| AI-A-16 | `Privacy: user`, `user_privacy: perform`, on a trunk whose RT-5 `pass_headers` omits every application-prefixed header | All six §5.3 headers removed by this spec and the application-prefixed ones by RT-5 — disjoint field sets, neither mechanism aware of the other (A33) |
| AI-A-17 | Response carrying `Privacy: user` and `Server: SuperPBX/4.2`, `user_privacy: perform` | `Server` deleted — RFC 5379 §5.1.12 (A35) |
| AI-A-18 | Response carrying `Privacy: user` and `Warning: 399 pbx.acme.example "Media downgraded"` | `Warning: 399 "Media downgraded"` — hostname deleted, warn-code and warn-text untouched (A35; §5.1.16) |
| AI-A-19 | Response carrying `Privacy: user` and `Warning: 399 "no hostname here"` | Forwarded byte-identical — nothing to anonymise (A35) |
| AI-A-20 | Response carrying `Privacy: user` and `Call-Info`, `Organization`, `Reply-To` | All three deleted — Table 1 marks them `Rr`, so they are reachable in this direction too (A35) |
| AI-A-21 | Response carrying `Privacy: user`, `From` in the response's own copy of the header | `From` **not** touched — Table 1 marks `From` `R` (A35) |
| AI-A-22 | Request carried `Privacy: user`; response carries **no** `Privacy` header and a `Server` header | `Server` forwarded — the caller's request does not strip the callee's headers. The A35 twin of `AI-P-11` (A35, A13) |
| AI-A-23 | Response carrying `Privacy: user`, trunk `user_privacy: decline` | All five forwarded unchanged — `Decline` performs none of it in either direction (A33, A35) |
| AI-A-24 | Response carrying `Privacy: user;critical` and a `Server` header | `Server` deleted and the response **forwarded** — never rejected; RFC 3323 §4.2 declines criticality on responses (A35) |
...
| Which **application-prefixed** headers reach a given carrier | RT-5's per-trunk egress header allowlist — the same grain and the same obligation, over a disjoint field set. It does not reach standard SIP headers, so it is not a control over anything this spec decides |
| An **unconditional** "this carrier never receives a `P-Asserted-Identity`", overriding even `Privacy: none` | **Nobody today — this is a known gap, not a deferral.** `A19` explains why no field here can be it, and RT-5's prefix scope means its allowlist cannot. Closing it needs a per-trunk unconditional egress removal for *standard* headers: a separate control, deliberately, because it knowingly violates RFC 3323 §4.2's `MUST NOT` for `none` and belongs where a reviewer sees that rather than inside a privacy default. Whether the platform should offer it at all is a product question — a deployment that needs it is asserting a contract with a carrier over a caller's explicit request |
