...
- [ ] **Failing-first**: add a commented-out invocation of a proof script to `scripts/gate.sh` and
      show `check-site.py` accepts it as gated. That is the substring test failing open, and it must
      be red after.
- [ ] "Runs in CI" means an invocation, not a mention. Parse a real invocation out of the shell and
      the workflow YAML, or move the declaration into a form that cannot be faked.
- [ ] At least `scripts/e2e-call.sh` actually runs in CI. It is the one proof with no external
...
- [ ] The proofs that genuinely cannot run in CI (`k8s-two-node-call.sh` needs a cluster) keep their
