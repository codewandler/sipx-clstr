...
**M0 is all but complete** (released as 0.2.0): the four load-bearing specs live in
`docs/specs/` — proxy behavior, location service, affinity token, hook framework — and the
deterministic-harness design is accepted, its upstream split recorded in the
[ledger](../upstream.md). The one remaining M0 story is `CX-1`: file the upstream stories in
the sipx repo, now that every decision those filings record has been made. `CX-2` then opens
M1 with the Cargo workspace. There is no Rust code yet. **The generated lists below group by
epic and do not order by priority** — `/track:next` reports the true top; do not take the
first rendered row.
