...
- [ ] `RegisterCommand`/CAS semantics: per-AoR serialization, idempotent retry, wildcard removal, Call-ID/CSeq comparison, `Min-Expires`/423 policy, per-tenant binding quotas, and the complete-set response — each with vectors.
