...
## Acceptance
- [ ] Fields specified with byte layout: format version, key id, tenant, home shard, edge affinity, direction (which side of the dialog the Route position faces), media node, policy version, expiry, nonce, authentication tag — with mint → encode → parse → verify round-trip vectors.
...
- [ ] Replay semantics are normative: re-presenting the same token on every mid-dialog request 
