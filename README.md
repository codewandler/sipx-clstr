...
<p align="center">
  ...
  <a href="docs/reference/conformance.md"><img src="https://img.shields.io/badge/vectors-219%2F621%20proved-2F3A45" alt="Conformance: 219 of 621 vector rows proved"></a>
...
>
> What does *not* exist yet is a **deployable affinity path**, so one address in front of both nodes
> will not work. The token library and a two-edge `Record-Route`/`Route` round trip are proved in
> deterministic simulation, and configuration loads and validates the key set; the running node does
> not apply those keys or deliver through a connection owner yet. Nor do trunks, media control, a
> user-credential store, or the operator exist. If you need a production clustered proxy today, this
> is not it.
...
`REGISTER` whose Request-URI or address-of-record names a domain the tenant does not serve is
answered **`404`**. Declare the domain your phones actually use, or leave `domains` out to serve
any.
...
| | |
|---|---|
| **Working** | **Two nodes sharing one registrar.** RFC 3261 §16 forwarding with forking; `ACK` and in-dialog requests routed by the dialog's `Route` set and remote target rather than by an address-of-record lookup; `REGISTER` over a compare-and-swap location store, in memory or in PostgreSQL; configuration by one cluster-scoped document in YAML, JSON or TOML, with the tenant's served domains, binding quota and expiry bounds enforced from it and a bounded number of in-flight transactions answering `503` above it; media flowing directly between endpoints. Register through one node, be called through another — proved with `sipx` CLI phones, with audio, locally and on Kubernetes |
| **In the engines but not on the wire** | Read this row before trusting the one above. The pure decision cores model more than the driver performs, and the gap is **not** covered by vector rows that prove effect *production* — a row proves what the engine emits, never that a socket carried it. Matched `CANCEL` and Timer C are produced as effects and then discarded by the driver, so a Timer C is armed with the right value and never fires (`PX-12`). Outbound target resolution returns a UDP target and refuses a hostname outright, so there is no transport selection and no RFC 3263 (`RT-12`). Declared roles do reach the released dispatcher — a node derives its capability set from them and answers `405` for a method they do not wire — while the refusal shape, the counted `ACK` and the `echo` runtime are still open (`DP-13`). The probe still builds address-of-record-shaped `ACK` and `BYE`, so a passing probe run proves a second lookup rather than a dialog route (`ET-7`). The affinity-token library and `Record-Route`/`Route` round trip are proved across two simulated edges. Configuration now loads and validates the key set, but the running node does not apply it and no connection-owner delivery hop exists yet (`DP-17`, `AF-7`) |
| **Written** | **Thirteen specifications** carry normative rules and byte-level test-vector tables registered with the checker, covering proxy and registrar behavior, authentication, affinity and membership, configuration, routing policy, media control, probing, optional session service, and the `SipxCluster` resource. Two more — cluster membership and the owner RPC — are normative and deliberately register no vector prefix; the [conformance report](docs/reference/conformance.md) names them and why |
| **Proved by** | The deterministic multi-node harness — seeded, virtual-time, byte-identical on replay — plus a real-socket end-to-end test against `sipx` CLI phones, which runs in CI on every push. That is a **same-kernel, separate-process integration test**: the phone is built from the kernel checkout this repository pins, so it proves two processes agreeing over real UDP, not two implementations agreeing (an independent interop target is `CF-3`, and it does not exist yet). The [conformance report](docs/reference/conformance.md) is generated, not written: **219 of 621 vector rows proved**, 16 covered for shape only, 386 deferred, each naming a reason and an owner. A row proves what the engine *emits*; where the driver does not perform that effect, the row above says so |
| **Accepted but not applied** | Seventeen sections of the schema — `profile`, `management`, `keys`, `shardMap`, `registrar`, `normalisation`, `trunk`, `domain`, `destinationSet`, `routeRule`, `ingress`, `rateLimit`, `nat`, `mediaPool`, `observability`, `probe`, `echo` — remain without runtime application, as do a listener's `tls`, `maxConnections` and `connectionLifetime`. `keys` and `shardMap` are the important stricter subset: they load into typed values and are validated, while most of the other sections are only recognised. The node logs the exact unapplied paths at startup, and the security-relevant ones on their own line. `tenant[].auth` has **left** this list: it is applied, and a document this build cannot honour is refused rather than accepted |
| **Not yet** | **One address in front of the cluster** — the Route-token round trip works and configuration loads and validates its keys, but the running node does not apply that key set and a request still cannot reach the edge that owns a client's connection. Also: trunk routing, media control, a user-credential store (so there is no authenticated registrar, only an open one or a refusal), registrar sharding, the operator and chart |
| **Built on** | [sipx](https://github.com/codewandler/sipx) 1.0.0-beta.5 — the SIP kernel this platform orchestrates, pinned to a tag rather than a branch. Protocol logic belongs there; this repo adds clustering |
